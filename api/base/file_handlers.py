...
def handle_student_csv(file):
    ...
    for _, row in data.iterrows():
        usn = row['USN']
        sap_id = row['STUDENT NUMBER']
        name = row['STUDENT FULL NAME']
        phone = row['STUDENT CONTACT']
        email = row['RVCE Mail ID']
        year = email[len(email)-14:len(email)-12]
        branch = email[len(email)-16:len(email)-14].upper()
        year_joined = datetime.strptime(year, '%y').date()
        personal_email = row['STUDENT EMAIL']
        ...
        if created:
            ...
            obj, created = Student.objects.get_or_create(
                usn=usn, sap_id=sap_id, name=name, phone=phone, email=email, personal_email=personal_email, year_joined=year_joined, branch=branch, user=user)
